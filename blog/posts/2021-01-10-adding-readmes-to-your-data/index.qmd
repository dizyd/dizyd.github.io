...
The next step is to add two example entries of each variable to this `temp0` data.frame *if* the argument `add_examples` equals `TRUE`. The lines to do this inside the `if` statement will:
...
- and return everything as vector (`unlist()`).  

Afterwards we add this new vector `temp_info` to the data.frame as a new column called `Description`
